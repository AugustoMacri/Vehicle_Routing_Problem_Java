...
def compare_best_results():
    ...
    print(f"Analisando {len(mono_files)} arquivos mono e {len(multi_files)} arquivos multi...")
    ...
    if best_mono_fitness < best_multi_fitness:
        ...
        improvement = (best_multi_fitness - best_mono_fitness) / best_multi_fitness * 100
    else:
        winner = "Multi-Objetivo"
        improvement = (best_mono_fitness - best_multi_fitness) / best_mono_fitness * 100
    ...
    if use_generations:
        ...
        mono_indices = [i/(len(best_mono_values)-1) * 100 for i in range(len(best_mono_values))]
        multi_indices = [i/(len(best_multi_values)-1) * 100 for i in range(len(best_multi_values))]
    ...
    # Configurar gráfico
    plt.title('Comparação dos Melhores Resultados: Mono vs Multi-Objetivo', fontsize=16)
    plt.ylabel('Fitness', fontsize=14)
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.legend(fontsize=12)
    
    # Adicionar anotação sobre qual foi melhor
    plt.annotate(f'Vencedor: {winner}\nMelhoria: {improvement:.2f}%', 
    ...
    output_file = os.path.join(output_dir, f'best_results_comparison_{timestamp}.png')
    ...
    summary_file = os.path.join(output_dir, f'best_results_summary_{timestamp}.txt')
