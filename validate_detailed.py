...
def validate_route_detailed(route_idx, route, customers):
    ...
    # Início no depot
    print(f"{'START':<6} {'Depot(0)':<25} {'-':<8} {'-':<8} {current_time:<10.2f} {'[0, 230]':<20} {'-':<8} {'-':<8} {current_time:<10.2f} {'🏁 Início':<15}")
    ...
    print("-" * 140)
    print(f"{'END':<6} {f'{current_customer.id} → Depot(0)':<25} {dist:<8.2f} {travel_time:<8.2f} {arrival_time:<10.2f} {'[0, 230]':<20} {'-':<8} {'-':<8} {arrival_time:<10.2f} {status_return:<15}")
...
def main():
    ...
    for route_idx, route in enumerate(routes):
        violations, dist, demand = validate_route_detailed(route_idx, route, customers)
        all_violations.extend(violations)
        total_distance += dist
        total_demand += demand
        
        if demand > vehicle_capacity:
            print(f"   ⚠️  ATENÇÃO: Capacidade excedida! ({demand} > {vehicle_capacity})")
    ...
    if all_violations:
        ...
        for v in all_violations:
            print(f"{v['route']:<8} {v['customer']:<10} {v['arrival']:<12.2f} {v['deadline']:<12} {v['delay']:<12.2f}")
    else:
